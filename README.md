//本项目基于 ChatDev 多角色协同框架，搭建面向个人开发者的全栈开发 AI Agent 系统，核心用于深度探索小米 MiMo-V2.5 系列大模型在全栈开发场景下的 Agent 工程化应用、能力适配与效果优化。针对个人开发者在全栈项目落地过程中面临的前后端技术栈跨度大、重复编码工作占比高、跨模块调试排错周期长、长上下文代码理解与联动修改能力不足、单元测试覆盖不全等核心痛点，本项目将深度对接 MiMo 大模型的核心能力，包括 128K 超长上下文窗口、精准的多轮工具调用、多语言代码生成与合规校验、中文需求语义的深度理解、多模态需求解析等，构建「需求输入→架构拆解→代码生成→自动化校验→缺陷定位→迭代优化」的全链路闭环开发工作流。通过 MiMo 大模型的能力适配，本项目将有效解决个人开发者全栈开发中的效率瓶颈，预期可将单项目的开发周期缩短 60% 以上，代码调试排错耗时降低 70%，单元测试覆盖率提升至 90% 以上，大幅降低全栈开发的技术门槛与时间成本。本次申请小米 MiMo 大模型 Token 额度，将核心用于本项目的模型适配调试、多场景压力测试、功能迭代优化与长链路推理场景验证。
# ChatDev 2.0 - DevAll
